...
class Karetaker_Harden {
	...
	private static function apply_user_enum() {
		...
		add_filter( 'oembed_response_data', array( __CLASS__, 'filter_oembed_author' ), 10, 1 );
	...
	public static function block_author_enum( $wp ) {
		...
		$digit_author = ( '' !== $author && ctype_digit( $author ) );
		if ( ! $digit_author && '' === $author_name ) {
			return;
		}
	...
	/**
	 * Strips author identity from public oEmbed payloads.
	 *
	 * @since 1.0.0
	 * @param array $data oEmbed response data.
	 * @return array
	 */
	public static function filter_oembed_author( $data ) {
	...
	/**
	 * Returns a generic login error so valid usernames are not confirmed.
	 *
	 * @since 1.0.0
	...
	public static function filter_login_errors( $error ) {
		...
		return __( 'Invalid credentials.', 'karetaker' );
	}

	/**
	 * Blocks guest access to prepared user REST entities (belt-and-braces with route removal).
	 *
	 * @since 1.0.0
	...
	public static function filter_rest_prepare_user( $response, $user, $request ) {
		...
		return new WP_Error(
			'karetaker_rest_user_blocked',
			__( 'User listing is disabled.', 'karetaker' ),
			array( 'status' => 401 )
		);
	}
	...
	/**
	 * Whether the protection behind a harden key is live on this request.
	 *
	 * @since 1.0.0
	 * @param string $key Harden toggle key.
	 * @return bool
	 */
	public static function probe_ok( $key ) {
		$key = sanitize_key( $key );

		if ( 'file_editor' === $key && defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT ) {
			return true;
		}
		if ( 'registration' === $key ) {
			return '0' === (string) get_option( 'users_can_register' );
		}
		if ( 'xmlrpc' === $key ) {
			return ! apply_filters( 'xmlrpc_enabled', true ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Core filter.
		}

		if ( ! in_array( $key, self::KEYS, true ) || ! self::is_on( $key ) ) {
			return false;
		}

		switch ( $key ) {
			case 'headers':
				return (bool) self::$booted;
			case 'xmlrpc':
				return ! apply_filters( 'xmlrpc_enabled', true ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			case 'file_editor':
				return false;
			case 'user_enum':
				return (bool) has_filter( 'rest_endpoints', array( __CLASS__, 'filter_rest_endpoints' ) );
			case 'version':
				return (bool) has_filter( 'the_generator', array( __CLASS__, 'empty_generator' ) );
			case 'registration':
				return '0' === (string) get_option( 'users_can_register' );
			case 'app_passwords':
				return (bool) has_filter( 'wp_is_application_passwords_available_for_user', array( __CLASS__, 'filter_app_passwords_user' ) );
		}
		...
		switch ( $key ) {
			...
			case 'user_enum':
				...
				return has_filter( 'rest_endpoints', array( __CLASS__, 'filter_rest_endpoints' ) )
					? __( 'REST, author URLs, oEmbed, login hints, and users sitemap blocked', 'karetaker' )
					: __( 'Off', 'karetaker' );
